...
public class BDD4jTestWatcher implements TestWatcher
{
    ...
    public void testAborted(final ExtensionContext context, final Throwable cause)
    {
        publishEvent(new ScenarioTestAbortedEvent(
                ...
                context.getTestClass()
                       .map(BDD4jTestWatcher::extractFeatureName)
                       .orElse(""),
                context.getTestClass()
                       .map(BDD4jTestWatcher::extractUserStory)
                       .orElse(""),
                context.getTestMethod()
                       .map(BDD4jTestWatcher::extractScenarioName)
                       .orElse(""),
                context.getTags()));
    }
    ...
    {
        return Optional.ofNullable(clazz.getAnnotation(Feature.class))
                       .map(Feature::value)
    ...
    {
        return Optional.ofNullable(clazz.getAnnotation(UserStory.class))
                       .map(UserStory::value)
    ...
    {
        return Optional.ofNullable(method.getAnnotation(Scenario.class))
                       .map(Scenario::value)
